...
MIN_SEQ_LEN = ''
CNN_layer, LR_model = '', ''
if load_LR_model: LR_model = '_LR'
MODEL = ['N','latt','gatt','glatt']
...
act_index_mapping = activity_index_mapping(mapping_file)
...
with tf.name_scope('Embedding_layer'):
    ...
    #batch_embedded = tf.layers.batch_normalization(batch_embedded)
    ...
    with tf.Session(config=session_conf) as sess:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for b in tqdm(range(num_batches), ascii=True):
                ...
                loss_tr, lab, pred, _, summary = sess.run([loss, target_ph, y_hat, optimizer, merged],
                                                    feed_dict=feed_dict)
                ...
                if b % num_eval == (num_eval - 1):
                    ...
                    loss_list,lab_list,pred_list,sig_acts,emb_list,user_list,act_scores,all_activities = [],[],[],[],[],[],[],[]
                    ...
                    for c in tqdm(range(num_batches_test), ascii=True):
                        ...
                        test_loss, lab, pred, alp, beta, u_emb, summary = sess.run([loss, target_ph, y_hat, alphas, betas, attention_output, merged],
                                                                 feed_dict=feed_dict)

                        user_list += list(u_batch)
                        emb_list += list(u_emb)                        
                        pred_list += list(pred)
                        lab_list += list(lab)
                        ...
                        sig_act, activities, act_score = discover_significant_activity(act_index_mapping, alp, x_batch, y_batch, seq_len, 3)
                        act_score_mapping_p, act_score_mapping_a = activity_score_mapping(alp, x_batch, y_batch, act_score_mapping_p, act_score_mapping_a, seq_len)
                        sig_acts += sig_act
                        all_activities += activities
                        act_scores += act_score
                    ...
                    eval_index += 1
                    pred_list,lab_list,loss_list,emb_list,user_list,all_activities,sig_acts = [],[],[],[],[],[],[]
